...
The package is capable of running a couple of different models. For our project, I added the livestock resilience model to the package. You can review it's parameters and specifications by typing
...
```{r}
l <- init_landscape(states = c("1","0"), cover = c(0.2,0.8))
plot(l)
```
...
pp$p <- 0.2
pp$L <- 4
run <- ca(l, model = livestock, parms = pp, t_max = 400)
plot(run)
```
